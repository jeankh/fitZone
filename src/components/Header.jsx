    ...
    >
      ...
      >
        <nav className="flex flex-col gap-2 p-6" onClick={e => e.stopPropagation()}>
          ...
          <button onClick={() => { setMobileMenuOpen(false); navigate('/programs') }} className="mt-4 bg-gradient-to-r from-brand to-brand-dark text-white px-6 py-4 rounded-xl text-lg font-semibold">
            {t('nav.getBooks')}
          </button>
        </nav>
      </motion.div>
